...
export function openModal({ title, body, buttons = [{ label: '關閉' }] }) {
  ...
  dialog.innerHTML = `
    <h3 style="font-family:var(--font-serif);font-size:18px;margin-bottom:12px">${title}</h3>
    <div style="font-size:13px;color:var(--ink);line-height:1.7;margin-bottom:20px">${body}</div>
  ...
  buttons.forEach(b => {
    ...
    btn.style.cssText = b.primary
      ? 'padding:10px 20px;background:var(--ink);color:var(--paper);border:none;border-radius:8px;cursor:pointer;font-size:14px'
      : 'padding:10px 20px;background:var(--paper);color:var(--ink);border:1.5px solid var(--border);border-radius:8px;cursor:pointer;font-size:14px';
